...
class Searcher:
    ...
    def __init__(self,depth_limit):
        """constructs a new Searcher object by initializing states, num_tested, depth_limit
        """
        self.states = []
        self.num_tested = 0
    ...
    def add_states(self, new_states):
        """that takes a list State objects called new_states, and that processes the elements of new_states one at a time 
        """
        for ste in new_states:
            if self.should_add(ste):
                self.add_state(ste)
...
class DFSearcher(Searcher):
    ...
    """
    def next_state(self):
        """method should follow LIFO (last-in first-out) ordering – choosing the state that was most recently added to the list.
        """
        s = self.states[-1]
        self.states.remove(s)
        return s
